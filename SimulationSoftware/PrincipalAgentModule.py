...
外部效用 U0（var_u0）：输入框，用来指定参与约束中的最低外部效用值。
成本系数 c（var_c）：输入框，用于设置代理人努力成本的系数。
产出价格 P（var_p）：输入框，用来指定完成任务或产生产出时的单价。
w范围（var_wmin、var_wmax、var_wstep）：三个输入框，分别代表合同中固定工资部分的最小值、最大值和步长，用于网格扫描。
b范围（var_bmin、var_bmax、var_bstep）：三个输入框，分别代表合同中绩效工资部分的最小值、最大值和步长，用于网格扫描。
可视化类型（var_chart）：下拉框，选择“利润-3D”、“利润-热力图”、“效用-3D”或“效用-热力图”，用于指定数据展示方式。
"""

class PrincipalAgent(ttk.Frame):
