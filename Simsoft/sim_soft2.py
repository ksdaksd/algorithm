...
class RiskPreferenceModule(BaseModule):
    ...
    def on_pref_change(self):
        ...
        defaults = {"averse": 0.5, "neutral": 1.0, "preferring": 2}
    ...
    def compute_utility(self):
        ...
        if pref == "averse":  # ln w
            ...
            if alpha <= 1:
                alpha = 2.0
            u = lambda x, a=alpha: x ** a
            u_inv = lambda y, a=alpha: y ** (1 / a)
            u_name = f"w^{alpha:.2f}"
        ...
        Ew = p * w1 + (1 - p) * w2 # 期望收益
        EU = p * U1 + (1 - p) * U2 # 期望效用
        CE_w = u_inv(EU) # 与期望收益等价期望效用

        # ---------- 打印 --------------------------------------------
        logbox.insert("end",
                      f"[第{self.eu_count}次] 风险类型: {pref}\n"
                      f"  效用函数: {u_name}\n"
                      f"  E(w) = {Ew:.4f}\n"
                      f"  EU   = {EU:.4f}\n"
                      f"  CE   = {CE_w:.4f}\n\n")
    ...
    # ================================================================
    # 2) 示意图绘制 —— y 轴随输入自适应且在不同风险类型间保持一致
    # ================================================================
    def plot_utility(self):
        ...
        # ---------- 统一 y 轴范围（随输入而定） -----------------------
        xs_sample = np.linspace(min(w1, w2), max(w1, w2), 400)
        # 三条基准曲线
        alpha_eff = alpha if alpha > 1 else 2
        base_vals = np.hstack([
            U1,
            U2
        ])
        # 所有标注点
        point_vals = np.array([U1, U2, EU, U_Ew])
        y_min, y_max = base_vals.min(), base_vals.max()
        y_min = min(y_min, point_vals.min())
        y_max = max(y_max, point_vals.max())
        ...
        self.ax.plot([w1, w2], [U1, U2],
                     color="gray", lw=1.0, label="自然连线")

        # —— 点位布局 ------------------------------------------------
        pts = [("A", w1, U1), ("D", w2, U2)]
        if pref == "averse":  # 风险厌恶
            pts += [("B", Ew, EU), ("C", Ew, U_Ew), ("E", CE_w, EU)]
        elif pref == "preferring":  # 风险爱好
            pts += [("B", Ew, U_Ew), ("C", Ew, EU), ("E", CE_w, EU)]
        else:  # 风险中性
            pts += [("B", Ew, EU), ("C", w2, U2)]  # C=D（仍保留 C 标注）

        color_map = {"A": "black", "B": "royalblue", "C": "firebrick",
                     "D": "black", "E": "seagreen"}
        for lbl, x, y in pts:
            self.ax.scatter(x, y, color=color_map.get(lbl, "black"), zorder=5)
            self.ax.text(x, y, f" {lbl}", va="bottom")

        # —— 虚线 ----------------------------------------------------
        x_min, x_max = xs.min(), xs.max()
        for _, x, y in pts:
            self.ax.vlines(x, y_min, y, color="gray", ls="dashed", lw=0.8)
        for y in {y for *_, y in pts}:
            self.ax.hlines(y, x_min, x_max, color="gray", ls="dashed", lw=0.8)

        # —— 装饰 ----------------------------------------------------
        self.ax.set_ylim(y_min, y_max)
        self.ax.set_xlabel("财富  w")
        self.ax.set_ylabel("效用  U(w)")
        title = {"averse": "风险厌恶", "neutral": "风险中性", "preferring": "风险爱好"}
        self.ax.set_title(title.get(pref, "效用示意图"))
...
class AgentAI:
    ...
    def cost_set(self, res_high, res_low):
        # 教学演示：仍可随机，也可由外部直接赋值
        self.cost_high_var = random.randint(round(res_low / 3), round(res_high / 3))
        self.cost_low_var = random.randint(0, round(res_low / 3))
...
class PrincipalAgentModule(BaseModule):
    ...
    def system_propose_contract(self):
        ...
        if self.cost_high_var.get()<self.cost_low_var.get():
            ...
            self.w1_sys, self.w2_sys = w1, w2
            self.log.insert('end', f"系统(委托人)提出合同: 高产出工资 = {w1}, 低产出工资 = {w2}\n")
            self.log.see('end')
...
class MoralHazardModule(BaseModule):
    ...
    def _write_intro(self):
        intro = (
            ...
            "  γ=1 为对数效用，γ>1 风险厌恶，γ=0 风险中性，γ<0 风险爱好。\n\n"
        )
