...
class App(tk.Tk):
    def __init__(self):
        ...
        # 应用现代主题（淡色主题），如可用则使用ttkbootstrap，否则退回默认主题
        if Style:
            style = Style(theme="flatly")  # 使用ttkbootstrap的“flatly”扁平风格主题
        else:
            style = ttk.Style()
            style.theme_use("clam")       # 使用内置的“clam”主题作为替代（较现代的扁平风格）
        # 统一全局字体（使用清爽的无衬线字体）
        default_font = ("Microsoft YaHei", 11)
        self.option_add("*Font", default_font)       # 设置所有组件的默认字体
        style.configure(".", font=default_font)      # 确保ttk组件也使用该默认字体
    ...
    # ...（generate_report_filename 方法保持不变）...
...
class MainMenu(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        for txt, frame in buttons:
            # 使用统一风格的ttk按钮，并增加适当的留白
            ttk.Button(self, text=txt, width=30,
                       command=lambda f=frame: controller.show_frame(f)
                       ).pack(pady=10)
        # （按钮的宽度和布局保持统一，使界面整齐美观）

class BaseModule(ttk.Frame):
    def __init__(self, parent, controller):
        super().__init__(parent)
        self.controller = controller
        # 工具栏区域，包含返回/主页/查看报告按钮
        toolbar = ttk.Frame(self)
        toolbar.pack(fill="x", padx=10, pady=10)  # 增加上下和左右留白
        ttk.Button(toolbar, text="返回",
                   command=lambda: controller.show_frame(MainMenu)
                   ).pack(side="left", padx=5)
        ttk.Button(toolbar, text="返回主菜单",
                   command=lambda: controller.show_frame(MainMenu)
                   ).pack(side="left", padx=5)
        ttk.Button(toolbar, text="查看报告",
